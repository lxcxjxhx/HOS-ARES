...
| | |
| --- | --- |
| 🧠 **AI 核心** | DeepSeek-Reasonix 官方源码交叉编译（linux/arm64，非 release 二进制） |
| 📦 **零等待开箱** | python3 + 177 个依赖 + 8 个安全工具全部预装进 rootfs，首次解压即用，无需联网安装 |
| 🔌 **MCP 原生接入** | 5 个 MCP server（Argus/PentestGPT/RepoAudit/Tengu/MCTS）+ 3 个 CLI 工具，reasonix 启动即自动发现 |
| 🎯 **统一模型** | 所有工具 LLM 统一继承 `reasonix setup` 的 DeepSeek Key，零额外配置 |
...
├─────────────────────────────────────────────────────┤
│  proot（GPL 组件）→ Alpine Linux 3.20 (arm64)       │
│    ├── python3.12 + 177 wheels（全部预装）           │
│    ├── DeepSeek-Reasonix（源码构建）                 │
│    └── /opt/ 8 个安全工具（MCP / CLI）               │
├─────────────────────────────────────────────────────┤
│  DeepSeek API（reasonix setup 统一配置）             │
└─────────────────────────────────────────────────────┘
```
...
- 不适用手机 proot 的未集成：metasploit / docker / ollama；frida 类需设备 Root
- 首次启动解压约 1-2 分钟；APK 约 570MB（预装环境的体积代价）
